...
- Use PaddleOCR-VL as the general visual route for every supported image task, including people, buildings, objects, actions, natural scenes, documents, OCR, historical material, tables, formulas, seals, UI, photos, products, charts, and counting. Answer the user's normal visual question from Paddle observations instead of limiting the response to OCR. The current AI Studio community limit is 3000 pages per user per model per day and at most the first 100 pages of one file; tell the user that limits can change.
